...
void getCraftingOutput (PlayerData *player, uint8_t *count, uint16_t *item) {
  ...
  for (i = 0; i < 9; i ++) {
    if (player->crafting[i].count > 0) {
      filled ++;
      if (first == 10) first = i;
      else if (player->crafting[i].protocol_id != player->crafting[first].protocol_id) {
        identical = false;
      }
    }
  }

  uint16_t first_item = player->crafting[first].protocol_id;
  ...
  switch (filled) {
    ...
    case 2:
      switch (first_item) {
        case I_oak_planks:
          if (first_col != 2 && player->crafting[first + 1].protocol_id == I_oak_planks) {
            *item = I_oak_pressure_plate;
            *count = 1;
            return;
          } else if (first_row != 2 && player->crafting[first + 3].protocol_id == I_oak_planks) {
            *item = I_stick;
            *count = 4;
            return;
          }
        ...
        case I_charcoal:
        case I_coal:
          if (first_row != 2 && player->crafting[first + 3].protocol_id == I_stick) {
            *item = I_torch;
            *count = 4;
            return;
          }
        ...
        case I_iron_ingot:
          if (
            (
              ...
              player->crafting[first + 4].protocol_id == I_iron_ingot
            ) || (
              first_row != 2 && first_col != 0 &&
              player->crafting[first + 2].protocol_id == I_iron_ingot
            )
          ) {
            *item = I_shears;
            *count = 1;
    ...
    case 3:
      switch (first_item) {
        ...
        case I_snow_block:
          ...
          if (
            first_col == 0 &&
            player->crafting[first + 1].protocol_id == first_item &&
            player->crafting[first + 2].protocol_id == first_item
          ) {
        ...
        case I_netherite_ingot:
          ...
          if (
            first_row == 0 &&
            player->crafting[first + 3].protocol_id == I_stick &&
            player->crafting[first + 6].protocol_id == I_stick
          ) {
          ...
          if (
            first_row == 0 &&
            player->crafting[first + 3].protocol_id == first_item &&
            player->crafting[first + 6].protocol_id == I_stick
          ) {
      ...
      switch (first_item) {
        ...
        case I_snowball:
          ...
          if (
            ...
            player->crafting[first + 1].protocol_id == first_item &&
            player->crafting[first + 3].protocol_id == first_item &&
            player->crafting[first + 4].protocol_id == first_item
          ) {
        ...
        case I_netherite_ingot:
          if (
            ...
            player->crafting[first + 2].protocol_id == first_item &&
            player->crafting[first + 3].protocol_id == first_item &&
            player->crafting[first + 5].protocol_id == first_item
          ) {
    ...
    switch (first_item) {
        ...
        case I_netherite_ingot:
          ...
          if (
            ...
            player->crafting[first + 1].protocol_id == first_item &&
            player->crafting[first + 2].protocol_id == first_item &&
            player->crafting[first + 4].protocol_id == I_stick &&
            player->crafting[first + 7].protocol_id == I_stick
          ) {
    ...
    case 7:
      // Legging recipes
      if (identical && player->crafting[4].protocol_id == 0 && player->crafting[7].protocol_id == 0) {
        switch (first_item) {
      ...
      switch (first_item) {
        case I_oak_slab:
          if (
            identical &&
            player->crafting[1].protocol_id == 0 &&
            player->crafting[4].protocol_id == 0
          ) {
            *item = I_composter;
            *count = 1;
            return;
    ...
    case 8:
      if (identical) {
        if (player->crafting[4].protocol_id == 0) {
          // Center slot clear
          switch (first_item) {
        ...
        } else if (player->crafting[1].protocol_id == 0) {
...
void getSmeltingOutput (PlayerData *player) {
  ...
  uint8_t *material_count = &player->crafting[0].count;
  uint8_t *fuel_count = &player->crafting[1].count;

  // Don't process if we're missing material or fuel
  if (*material_count == 0 || *fuel_count == 0) return;

  uint16_t *material = &player->crafting[0].protocol_id;
  uint16_t *fuel = &player->crafting[1].protocol_id;

  // Don't process if we're missing material or fuel
  if (*material == 0 || *fuel == 0) return;

  // Furnace output is 3rd crafting table slot
  uint8_t *output_count = &player->crafting[2].count;
  uint16_t *output_item = &player->crafting[2].protocol_id;
